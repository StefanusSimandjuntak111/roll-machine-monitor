#!/usr/bin/env python3
"""
Complete Build Script for Monitoring Roll Machine v1.4.4
Creates a NSIS installer with full install/update support
Includes batch tracking, Supabase integration, and ERP integration features
"""
...
class BuildManager:
    """Manages the complete build process for version 1.4.4."""
    ...
    def test_imports(self):
        ...
        try:
            ...
            self.log_success("Core modules import successfully")
            ...
            # Test config loading (basic check)
            try:
                from monitoring.config import load_config
                config = load_config()
                if not isinstance(config, dict):
                    self.log_warning("Configuration loading returned non-dict, but continuing...")
                else:
                    self.log_success("Configuration file is valid")
            except Exception as e:
                self.log_warning(f"Config test skipped: {e}")
            
            # Try to import UI modules (may fail if dependencies not installed)
            try:
                from monitoring.ui.main_window import ModernMainWindow
                self.log_success("UI modules import successfully")
            except ImportError as e:
                self.log_warning(f"UI modules import skipped (dependencies will be bundled): {e}")
            
            # Try to import batch tracking (may fail if dependencies not installed)
            try:
                from monitoring.batch_manager import BatchManager
                self.log_success("Batch manager module found")
            except ImportError as e:
                self.log_warning(f"Batch manager import skipped: {e}")
            
            # Try to import Supabase client (may fail if dependencies not installed)
            try:
                from monitoring.supabase_client import SupabaseClient
                self.log_success("Supabase client module found")
            except ImportError as e:
                self.log_warning(f"Supabase client import skipped (will be bundled): {e}")
            
            # Try to import ERP client (may fail if dependencies not installed)
            try:
                from monitoring.erp_client import ERPClient
                self.log_success("ERP client module found")
            except ImportError as e:
                self.log_warning(f"ERP client import skipped (will be bundled): {e}")
            
            self.log_success("Import test completed (some dependencies will be bundled by PyInstaller)")
            return True
            
        except Exception as e:
            self.log_error(f"Critical import test failed: {e}")
            import traceback
            traceback.print_exc()
            return False
...
'''
    ...
    def create_nsis_installer(self):
        ...
        # Create NSIS script
        nsis_script = self.project_root / "installer_v1.4.3.nsi"
        
        nsis_content = f'''
; Monitoring Roll Machine v1.4.3 Installer
...
OutFile "Monitoring-Roll-Machine-v1.4.3-Setup.exe"
...
### 🔗 **ERP Integration** (NEW)
- **Stock Entry Submission**: Automatic submission of production data to ERP system
- **ERP Client**: Complete integration with ERPNext/Frappe framework
- **Offline Queue**: Automatic queuing when ERP is unavailable
- **Auto-Retry**: Automatic retry mechanism for failed submissions
- **Configurable Settings**: ERP URL, API keys, and warehouse settings
...
- **Enhanced Configuration**: ERP and Supabase credentials in config.json
- **Better Error Handling**: Graceful fallback when services unavailable
- **Improved Logging**: Better tracking of batch and ERP operations
- **Offline Queue**: Improved queue handling for ERP submissions
...
### ✅ **ERP Integration**
- Automatic stock entry submission
- Configurable ERP settings
- Offline queue for failed submissions
- Auto-retry mechanism
- Warehouse and BOM configuration
...
- Internet connection (optional, for Supabase and ERP)
...
### ERP Setup
1. Configure ERP settings in `config.json`:
   ```json
   {{
     "erp_url": "https://your-erp-instance.com",
     "erp_api_key": "your-api-key",
     "erp_api_secret": "your-api-secret",
     "erp_company": "Your Company",
     "erp_from_warehouse": "Source Warehouse",
     "erp_to_warehouse": "Destination Warehouse",
     "enable_erp_submission": true
   }}
   ```
...
### Configuration
- Configuration file with ERP and Supabase settings
...
- Supabase and ERP integration are optional
- No breaking changes to existing functionality

### New Features
- ERP integration can be enabled anytime
- Offline queue handles ERP submission failures
...
### ERP Integration
- ERPNext/Frappe framework support
...
- Offline queue for ERP submissions
...
"""
    ...
    def run(self):
        ...
        logger.info("  ✓ ERP integration for stock entry submission")
        ...
        print("  • ERP integration for stock entry")
