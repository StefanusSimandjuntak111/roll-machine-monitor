...
class JSKSerialPort:
    ...
    def __init__(
        ...
        # FIX: Remove lambda function creation - we'll create objects directly in open()
        # if simulation_mode:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if not self._serial or not self._serial.is_open:
                    ...
                    # FIX: Create serial object directly like in serial_tool.py
                    if self.simulation_mode:
                        self._serial = MockSerial(
                            port=target_port,
                            baudrate=self.baudrate,
                            timeout=self.timeout,
                            simulate_errors=self._simulate_errors
                        )
                        self._serial.open()
                    else:
                        # Use direct serial.Serial creation like in working serial_tool.py
